...
trait JdbcValamisReporterComponent
  extends ValamisReporterComponent
  with LrsDataContext
  ...
  lazy val  statementStorage: StatementStorage = new JdbcStatementStorage
  ...
  class JdbcStatementStorage extends StatementStorage {
    ...
    def findMaxActivityScaled(actor: Actor,
                              verbId: Verb#Id): Seq[(String, Option[Float])] = db withSession { implicit session =>
