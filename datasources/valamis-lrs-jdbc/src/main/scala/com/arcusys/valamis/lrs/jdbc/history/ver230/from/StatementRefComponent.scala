package com.arcusys.valamis.lrs.jdbc.history.ver230.from

import java.util.UUID

import com.arcusys.valamis.lrs.jdbc.history.ver230.from.row.StatementRefRow
import com.arcusys.valamis.lrs.tincan._

import scala.slick.jdbc._
import com.arcusys.valamis.lrs.jdbc.history.BaseComponent
import com.arcusys.valamis.lrs.jdbc.history.Helper._
...
trait StatementRefComponent {
  ...
  protected def getStatementObject(key: Long,
    ...
    case Constants.Tincan.Activity =>  {
      val activity = activities.find(_.key == key)

      require(activity.isDefined, s"Can't find Activity with key = $key")

      activity.get.toModel
    }
    case Constants.Tincan.StatementReference => {
      val ref = statementRefs.filter(_.id == key).head
    ...
    case Constants.Tincan.SubStatement => {
      val subStatement = subStatements.filter(_.key == key).head
