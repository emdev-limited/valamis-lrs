...
import java.util.UUID
  ...
  import com.arcusys.valamis.lrs.jdbc.history.ver230.from.row.DocumentRow
...
trait DocumentComponent { this: BaseComponent =>
  ...
  protected def getDocument(id: UUID)(implicit session: JdbcBackend#Session): Document = {
    ...
    doc getOrElse (throw new IllegalArgumentException(s"Can't find document with id = $id"));
  ...
  protected def getDocument(key: Long)(implicit session: JdbcBackend#Session): Document = {
    ...
    doc getOrElse (throw new IllegalArgumentException(s"Can't find document with key = $key"));
